...
var GameState, Map, addParticle, cubeData, height, renderer, stats, width;
...
addParticle = function(position) {
  var geometry, material, mesh, scene;
  scene = engine.scene();
  material = new THREE.ParticleBasicMaterial({
    size: 5,
    depthFalse: false
...
engine.on("update", function(dt) {
  var camera, cameraSpeed, intersects, objects, p, projector, raycaster, scene, vector;
  cameraSpeed = 100;
  ...
  if (mousePressed.left) {
    vector = new THREE.Vector3((mousePosition.x / App.width) * 2 - 1, -(mousePosition.y / App.height) * 2 + 1, 0);
    ...
    if (intersects.length > 0) {
      return addParticle(intersects.first().point);
    }
  }
});

engine.camera = function() {
  return engine.I.currentState.camera;
};
